...
def test_cria_usuario(client):
    response = client.post("/usuario", json={"nome_usuario": "Fulano", 'funcao':'Pessoa Fisica', 'login':'fulano3@gmail', 'senha':'123'})
...
def test_seleciona_usuario(client):
    response = client.get("/usuario/20")
    assert response.status_code == 200
    json_data = response.get_json()
    assert json_data["nome_usuario"] == "Fulano"
    assert json_data['login'] == 'fulano1@gmail'

def test_atualiza_usuario(client):
    response = client.put("/usuario/20", json={"nome_usuario": "Beltrano"})
    assert response.status_code == 200
    json_data = response.get_json()
    assert json_data["nome_usuario"] == "Beltrano"

def test_login_certo_usuario(client):
    response = client.post('/login', json={'login':'fulano@gmail', 'senha':'123'})
    assert response.status_code == 200

def test_login_errado_usuario(client):
    response = client.post('/login', json={'login':'fulano@gmail', 'senha':'1234'})
    assert response.status_code == 401

def test_deleta_usuario(client):
    response = client.delete("/usuarios/20")
